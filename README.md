...
- Web: [www.Zeon.org](https://www.Zeon.org)
- Mail: [dev@Zeon.org](mailto:dev@Zeon.org)
- GitHub: [https://github.com/zeonprojects/Zeon](https://github.com/zeonprojects/Zeon)
...
![](http://www.Zeon.org/user/pages/images/block_reward_by_calendar_month.png)
...
* Clone source code, change to the root of the source code directory and build:
        ...
        git clone https://github.com/zeonprojects/Zeon; cd Zeon; make release-static;
...
To list all available options, run `./bin/Zeond --help`.  Options can be
...
To run in background:
    ...
    ./bin/Zeond --log-file Zeond.log --detach

To run as a systemd service, copy
[Zeond.service](utils/systemd/Zeond.service) to `/etc/systemd/system/` and
[Zeond.conf](utils/conf/Zeond.conf) to `/etc/`. The [example
service](utils/systemd/Zeond.service) assumes that the user `Zeon` exists
and its home is the data directory specified in the [example
config](utils/conf/Zeond.conf).

If you're on Mac, you may need to add the `--max-concurrency 1` option to
zeon-wallet-cli, and possibly Zeond, if you get crashes refreshing.
...
`DNS_PUBLIC=tcp TORSOCKS_ALLOW_INBOUND=1 torsocks Zeond --p2p-bind-ip 127.0.0.1 --no-igd`

TAILS ships with a very restrictive set of firewall rules. Therefore, you need to add a rule to allow this connection too, in addition to telling torsocks to allow inbound connections. Full example:

`sudo iptables -I OUTPUT 2 -p tcp -d 127.0.0.1 -m tcp --dport 18081 -j ACCEPT`

`DNS_PUBLIC=tcp torsocks ./Zeond --p2p-bind-ip 127.0.0.1 --no-igd --rpc-bind-ip 127.0.0.1 --data-dir /home/your/directory/to/the/blockchain`

`./zeon-wallet-cli`

## Using readline

While `Zeond` and `zeon-wallet-cli` do not use readline directly, most of the functionality can be obtained by running them via `rlwrap`. This allows command recall, edit capabilities, etc. It does not give autocompletion without an extra completion file, however. To use rlwrap, simply prepend `rlwrap` to the command line, eg:

`rlwrap bin/zeon-wallet-cli --wallet-file /path/to/wallet`
...
`cd ~/Zeon/external/db_drivers/liblmdb && make`
